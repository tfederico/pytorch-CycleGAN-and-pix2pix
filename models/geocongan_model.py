...
class GeoConGANModel(BaseModel):
    """
    This class implements the GeoConGAN model, for learning image-to-image translation without paired data imposing geometric consistency.
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        For GeoConGAN, in addition to CycleGAN losses, we introduce geometric-consistency loss between the original mask and the mask of the generated images
    ...
    def __init__(self, opt):
        """Initialize the GeoConGAN class.
        ...
        if self.isTrain:  # if identity loss is used, we also visualize idt_B=G_A(B) ad idt_A=G_A(B)
            ...
            visual_names_A.append('mask_fake_B')
            visual_names_B.append('mask_B')
            visual_names_B.append('mask_fake_A')
        ...
        if self.isTrain:
            ...
            self.criterionGeo = torch.nn.BCELoss()
    ...
    def forward(self):
        ...
        if self.isTrain:
            with torch.no_grad():
                self.mask_fake_B = torch.sigmoid(self.SilNet(self.fake_B))
                self.mask_fake_A = torch.sigmoid(self.SilNet(self.fake_A))
